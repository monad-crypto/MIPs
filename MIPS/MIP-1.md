...
memory_cost = k * memory_size_words # k is a constant that is tbd
```
...
However, this proposal does introduce two limitations:

1. Contracts that depend on the exact quadratic memory cost behavior may break.
2. Contracts that allocate more than 8 MB of memory will now revert.
